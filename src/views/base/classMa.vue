  ...
  <div id="base-classMa">
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-button  type="text" @click="addClass()">
          新增班级
      ...
      <el-dialog :title="dailogTitle?'增加班级':'修改班级信息'" :visible.sync="dialogFormVisible">
        <el-form :model="form" :rules="rules" ref="form" v-if="dialogFormVisible">
          <el-form-item label="班级名称" :label-width="formLabelWidth" prop="className">
            <el-input v-model="form.className" autocompvare="off"></el-input>
          </el-form-item>
...
export default {
  ...
  created() {
    this.getClassData();//获取全部班级信息
    this.getCourserData();//获取专业课程信息
    this.getTeacherData();//获取老师数据
  },
  methods: {
    /**
     * 从api中获取所有班级的数据
    ...
    getTeacherData() {
      ...
      that.axios.get("User/GetTeachers").then(res => {
				// console.log("所有老师信息" ,that.teacherData);
    ...
    getCourserData() {
      ...
      that.axios.get("Class/GetAllCourse").then(res => {
				// console.log("所有课程信息",that.courseData)
    ...
    selectCourse() {
      var that = this;
      var course = that.courseData.find(item => {
    ...
    selectTeacher() {
      var that = this;
      var teacher = that.teacherData.find(item => {
